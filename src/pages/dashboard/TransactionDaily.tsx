...
const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'Transaction daily'
    }
  }
};

function TransactionDaily({ genesisHash }: { genesisHash: HexString }) {
  const { data } = useSWR<DailyWithChain[]>(`${ACCOUNTS_API}daily/tx/${genesisHash}`);
...
export default React.memo(TransactionDaily);
